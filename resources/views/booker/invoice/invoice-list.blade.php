...
@section('content')
                                                    ...
                                                    <td>
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                <a type="button" class="invDetail dropdown-item"
                                                                    data-invoice-id="{{ $item->id }}" data-toggle="modal"
                                                                    data-target="#invoiceModal">
                                                                    <i class="fas fa-list"></i>
                                                                    Detail
                                                                </a>

                                                                <a href="{{ url('booking/view-invoice/' . $item->id) }}"
                                                                    class="dropdown-item"> <i class="fas fa-eye"></i> View</a>

                                                                @if ($item->booking->booking_status != 'closed')
                                                                    @php $hasNonType1 = $item->bookingData->where('transaction_type', '!=', 1)->count() > 0; @endphp
                                                                    @if ($hasNonType1)
                                                                         @can ('edit booking') <a
                                                                            href="{{ url('customer-booking/' . $item->id . '/edit') }}"
                                                                ...
                                                                @can('delete invoice')
                                                                    <form
                                                                        action="{{ url('booking/' . $item->zoho_invoice_number . '/delete-invoice') }}"
                                                                        method="POST" style="display:inline;" class="delete-form">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="submit"
                                                                            class="dropdown-item delete-confirm text-danger"><i
    ...
    <!-- Modal -->
    <div class="modal fade" id="activeBookingModal" tabindex="-1" role="dialog" aria-labelledby="activeBookingLabel"
        aria-hidden="true">
...
@section('script')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function loadInvoices(search = '', page = 1) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.invoices && response.invoices.length > 0) {
                            ...
                            $.each(response.invoices, function (index, data) {
                                html += `
                                    <tr data-id="${data.id}">
                                        <td>${number}.</td>
                                        <td>${data.booking?.customer?.customer_name || 'N/A'}</td>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a type="button" class="invDetail dropdown-item" data-invoice-id="${data.id}" data-toggle="modal" data-target="#invoiceModal">
                                                        <i class="fas fa-list"></i> Detail
                                                    </a>
                                                    <a href="/booking/view-invoice/${data.id}" class="dropdown-item"> 
                                                        <i class="fas fa-eye"></i> View
                                                    </a>
                                                    <a href="/customer-booking/${data.id}/edit" class="dropdown-item"><i class="far fa-edit"></i> Edit</a>
                                                    <form action="/booking/${data.zoho_invoice_number}/delete-invoice" method="POST" style="display:inline;" class="delete-form">
                    ...
                    error: function() {
                        $('#bookingList').html(`<tr><td colspan="7" class="text-center text-danger">Error loading data</td></tr>`);
                        $('#paginationContainer').html('');
                    }
                });
            }
            ...
            $(document).on('click', '#paginationContainer .pagination a', function(e) {
                ...
                if (page) {
                    loadInvoices(currentSearch, page);
                }
            });
        });

...
@foreach ($payment as $item)
    <tr>
        ...
        <td>{{ $item->booking && $item->booking->invoice ? ($item->booking->invoice->zoho_invoice_number ?? '-') : '-' }}</td>
        ...
        <td>
            <div class="dropdown">
                ...
                <div class="dropdown-menu">
                    <button type="button" class="dropdown-item paymentHistory"
                        data-payment-id="{{ $item->id }}" data-toggle="modal" data-target="#paymentHistoryModal">
                        <i class="fas fa-eye"></i> View
                    </button>
                    @can('delete payment')
                    <form action="{{ route('payment.destroy', $item->id) }}" method="POST" style="display:inline;" class="delete-form">
                        @csrf
                        @method('DELETE')
